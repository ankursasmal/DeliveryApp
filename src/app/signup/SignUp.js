...
let data=await res.json();
 ...
 if(data.status==200){
  ...
  //  ***** importent thing ? through order paga a redit
  // if(redirect==props?.searchParams?.order){
  //   router.push('/order')
  // }
  // else{
  // router.push('/ ');
  ...
  return (
     <center>
      ...
      <h1 className="text-[3.5vw] font-semibold text-center self-center">SignUp</h1>
       <form onSubmit={handelpost} className='flex justify-center flex-col p-2 shadow-xl w-[60vw] md:w-[50vw] lg:w-[30vw]'>
    ...
    <button className="mt-4 px-4 py-2 text-blue-500" onClick={()=>router.push('/homelogin')}>Already Have a Acccount? Login</button>
